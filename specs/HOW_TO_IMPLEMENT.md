...
Write the steps in the plans as checkboxes.
...
Whenever you've completed a step in the plan, tick that box off.
...
When one feature/change from the original spec is done, check that off in the spec file, and move on to the next one. Compact the context if needed.
